...
class LineDetector:
    ...
    def __init__(self, topic):
        ...
        self.pixel_cnt_threshold2 = 0.2 * self.area_width2 * self.area_height2
        # =========
    ...
    def conv_image(self, data):
        ...
        # ==========
        red_lbound = np.array([0, 100, 0], dtype=np.uint8)
        red_ubound = np.array([30, 255, 255], dtype=np.uint8)

        yellow_lbound = np.array([15, 0, 0], dtype=np.uint8)
        yellow_ubound = np.array([50, 255, 255], dtype=np.uint8)

        green_lbound = np.array([50, 0, 170], dtype=np.uint8)
        green_ubound = np.array([80, 255, 255], dtype=np.uint8)
